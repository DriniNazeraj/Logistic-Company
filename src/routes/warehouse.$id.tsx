...
import { useEffect, useRef, useState, MouseEvent as RMouseEvent } from "react";
...
import { ArrowLeft, Plus, Trash2, Package as PackageIcon } from "lucide-react";

export const Route = createFileRoute("/warehouse/$id")({
  head: () => ({ meta: [{ title: "Warehouse layout — trans.al" }] }),
...
function WarehouseLayout() {
  ...
  const load = async () => {
    const [{ data: w }, { data: ss }, { data: ps }] = await Promise.all([
      ...
      supabase
        .from("packages")
        .select("id, package_code, product_name, section_id"),
    ]);
    setWarehouse(w);
    setSections(ss ?? []);
    setPackages(ps ?? []);
  ...
  const updateSection = async (s: Section, patch: Partial<Section>) => {
  ...
  return (
    <>
      <PageHeader
        ...
        description={warehouse.location || "Visual layout builder"}
        actions={
          <Link
            to="/warehouse"
          ...
          <div className="rounded-lg border border-border bg-card p-3">
            <div className="mb-3 flex items-center justify-between">
              <div className="font-mono text-xs text-muted-foreground">
                Canvas {warehouse.canvas_width} × {warehouse.canvas_height}
              </div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="text-center">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground/70">
                        Click "Add section" to start designing.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              {sel ? (
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  Click a section in the canvas to edit it. Drag to move, drag the corner handle to resize.
                </p>
              )}
            </div>
                ...
                <ul className="mt-2 space-y-1 text-sm">
                  {packages.filter((p) => p.section_id === sel.id).length === 0 ? (
                    <li className="text-xs text-muted-foreground">
                      No packages assigned. Assign one from the Packages page.
                    </li>
                  ) : (
                    packages
